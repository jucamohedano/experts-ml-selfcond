...
from modules.module_5_heatmaps import validate_against_human
...
def main() -> int:
    ...
    profile = pd.read_csv(MATRIX_DIR / "layer_profile_matrix.csv", index_col=0)
    concepts = list(jaccard.index)
    matrices = {"jaccard": jaccard.to_numpy(dtype=float),
                "layer_profile": profile.to_numpy(dtype=float)}
