...
### Cross-model check on the shared Richie-HSJ dataset (GPT-2 vs Qwen3)

Read on equal footing, both architectures on Richie-HSJ with the same sublayer restriction, the two levels separate on depth but not on concentration, and the pattern is the same in both models. The values below are recomputed directly from each run's `shannon_entropy_concepts.csv`, comparing the 8 category labels against the 196 specific concepts, with the entropy test one-sided (categories more concentrated) and the average-layer tests two-sided.

| Model | AP | Entropy, label vs concept (p) | Avg layer, label vs concept (p) | Trimmed avg, label vs concept (p) |
|---|---|---|---|---|
| GPT-2 | 0.5 | 3.24 vs 3.28 (0.20) | 26.4 vs 24.0 (0.038) | 29.5 vs 24.0 (0.12) |
| GPT-2 | 0.6 | 3.03 vs 3.07 (0.17) | 26.6 vs 22.0 (0.036) | 29.5 vs 20.2 (0.026) |
| Qwen3 | 0.5 | 4.39 vs 4.32 (0.88) | 112.5 vs 106.4 (0.074) | 137.4 vs 121.2 (0.13) |
| Qwen3 | 0.6 | 4.20 vs 4.16 (0.64) | 115.9 vs 103.5 (0.006) | 135.7 vs 112.1 (0.035) |

Two readings follow. First, category labels sit deeper in the network than their member concepts in both models, and the effect is clearest at AP 0.6 where both the full and the trimmed average layer separate (GPT-2 p 0.036 and 0.026, Qwen3 p 0.006 and 0.035), and it is directional at AP 0.5. So the depth contrast reported in the Qwen3 subsection above is not Qwen specific, it holds for GPT-2 on the same data too, and it is the more robust of the two level differences. Second, the entropy concentration contrast does not separate the levels on Richie-HSJ in either model, since all p values sit above 0.15 and the group means are within about 1 percent of each other. The significant concentration result of the GPT-2 150-concept run above is therefore specific to that dataset and its whole-model support, and it should not be generalized to the sublayer-restricted Richie-HSJ runs.

**Data integrity note.** The GPT-2 Richie-HSJ `main.log` records the entropy test as significant (U=392, p=0.0064, categories more concentrated), but that value cannot be reproduced from the `shannon_entropy_concepts.csv` and `shannon_entropy_categories.csv` in the same folder, which give p about 0.20 whether the 8 labels are excluded from or included in the concept group. The logged conclusion is inconsistent with the saved data, so it should be re-run before being cited anywhere. Separately, the entropy test in `compute_shannon_entropy` pools all 204 words into the concept sample (the category labels are left in) rather than the 196 specific concepts the formulation of subchapter 2.3 specifies, a code versus documentation mismatch that does not change the null here but should be aligned in the code.
