...
def run_overlap_analysis(
    ...
    threshold: float = 0.5,
    expertise_csv: pathlib.Path = None,
    expertise_dir: pathlib.Path = None,
    save_expert_sets: bool = False,
    save_matrix: bool = False,
    pattern: str = "**/expertise.csv",
):
    """
    Run the expert overlap analysis pipeline.
    
    Args:
        output_dir: Directory where to save expert sets and similarity matrices
        threshold: AP threshold for defining expert neurons
        expertise_csv: Path to a single expertise.csv file (mutually exclusive with expertise_dir)
        expertise_dir: Directory containing multiple expertise.csv files
        save_expert_sets: Save individual expert sets as .npy files
        save_matrix: Save similarity matrix as CSV file
    ...
    if not expertise_csv and not expertise_dir:
        print("Error: Must provide either expertise_csv or expertise_dir")
        sys.exit(1)
    ...
    if expertise_csv:
        print(f"Processing single file: {expertise_csv}")
        if not expertise_csv.exists():
            print(f"Error: File {expertise_csv} does not exist")
            sys.exit(1)
        ...
        print(f"Processing directory: {expertise_dir}")
        if not expertise_dir.exists():
            print(f"Error: Directory {expertise_dir} does not exist")
            sys.exit(1)
    ...
    if save_expert_sets:
        print("\nSaving individual expert sets...")
        expert_sets_dir = output_dir / "expert_sets"
        expert_sets_dir.mkdir(exist_ok=True)
    ...
    # Compute and save similarity matrix if requested or if multiple concepts
    if save_matrix or len(expert_sets) > 1:
        print("\nComputing similarity matrix...")
        ...
        save_similarity_matrix(
            ...
            concept_names,
            output_dir,
            f"similarity_matrix_tau_{threshold:.1f}.csv"
        )
        ...
        metadata_file = output_dir / f"analysis_metadata_tau_{threshold:.1f}.json"
        with metadata_file.open("w") as f:
            json.dump(metadata, f, indent=2)
...
def main():
    ...
    run_overlap_analysis(
        ...
        expertise_dir=args.expertise_dir,
        save_expert_sets=args.save_expert_sets,
        save_matrix=args.save_similarity_matrix,
        pattern=args.pattern,
    )
